		...
		<div id="box-inside-white">
			<div class="wrapper">
				<div class="row">
					...
					<div class="divdx">
						<div id="header-sub-title"><fmt:message key="utente_form_titolo"/></div>
						<div id="header-sub-action">
							<ul>
								...
								<li><a href="deleteUtente.do?id=${utente.id}"><img style="padding-right: 5px; padding-left: 3px;vertical-align: middle;" src="${pageContext.request.contextPath}/resources/icon/raster/gray_light/x_7x7.png" alt="<fmt:message key="del"/>" border="0"/><fmt:message key="del"/></a></li>
								<%--
						        ...
						        <ul>
						            <li>
						            	<spring:bind path="utente.userName">
						                	<label for="idl_${status.expression}" class="bold">User Name:</label>
											<input id="id_${status.expression}" type="text" name="${status.expression}" value="${status.value}" size="40"/>
										</spring:bind>
						            </li>
						            <%-- --%>
						            <c:if test="${utente.id le 0 and utente.userName ne 'admin'}">
							            <li>
							            	<spring:bind path="utente.password">
							                	<label for="idl_${status.expression}" class="bold">Password:</label>
												<input id="id_${status.expression}" type="password" name="${status.expression}" value="${status.value}" size="40"/>
											</spring:bind>
							            </li>
						            </c:if>
						            
						            <li>
						            	<spring:bind path="utente.cognome">
						                	<label for="idl_${status.expression}" class="bold">Cognome:</label>
											<input id="id_${status.expression}" type="text" name="${status.expression}" value="${status.value}" size="40"/>
										</spring:bind>
						            </li>
						            <li>
						            	<spring:bind path="utente.nome">
						            ...
						            <li>
						            	<div style="float: left; width: 100%;">
						            		<label for="idl_ruoli" class="bold">Ruoli:</label>
						            		<div style="float: left;">
						            			<ul>
						            			<c:forEach var="r" items="${utente.ruoli}" varStatus="idx">
						            				<li>
						            				<spring:bind path="utente.ruoli[${idx.index}].ruolo">
						            					<input type="hidden" name="${status.expression}" value="${status.value}"/>
						            				</spring:bind>
						            				<spring:bind path="utente.ruoli[${idx.index}].selezionato">     
                                        				<input type="hidden" id="hidden_id${status.expression}" name="_${status.expression}" value="visible"/>         
                                        				<input type="checkbox" id="id${status.expression}" name="${status.expression}" value="true"                                                 
