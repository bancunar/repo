		...
		<div id="box-inside-white">
			<div class="wrapper">
				<div class="row">
					...
					<div class="divdx">
						<div id="header-sub-title"><fmt:message key="conn_list_titolo"/></div>
						...
						<table class="stiletab" style="width: 95%;">
							<thead>
								<tr><th>Connessione</th><th>User Name</th><th>Password</th><th>Driver</th><th>URL</th><th>Tipo</th>
								<th style="width:20px; border-right:0;">&nbsp;</th></tr>
							</thead>
							<tbody>
								<c:forEach items="${lista}" var="item">
									<tr>
										<td><a href="showConnessione.do?id=${item.id}">${item.nome}</a></td>
										<td>${item.username}</td>
										<td>${item.password}</td>
										<td>${item.driver}</td>
										<td>${item.url}</td>
										<td>${item.tipo}</td>
										<td><img style="padding-right: 5px; padding-left: 3px;vertical-align: middle;" border="0"
